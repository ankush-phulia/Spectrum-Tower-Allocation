...
Types::Price_Bids checkClashBidwithState(Bid b, State &s) {
    //    returns list of bids in state with which b clashes + total cost of
    //    clsshing bids.
    ...
    for (auto it = bid_regs.begin(); it != bid_regs.end(); it++) {
        ...
        if (bid_state != -1 &&
            bids_clashing.find(bid_state) == bids_clashing.end()) {
            bids_clashing.insert(bid_state);
            // outfile << b.Bid_Id << " clashing with " << bid_state <<
            // std::endl;
    ...
    if (comp_bid < B && comp_bid > -1 &&
        bids_clashing.find(comp_bid) == bids_clashing.end()) {
...
bool addBidtoState(Bid b, State &s, Types::Price_Bids &clash, bool region) {
    ...
    if (clash.second.second.size() == 0) {
        ...
        // WORKS GOOD ONLY IF EACH BID HAS LOTSSS OF REGIONS. -> check avg!!!
        // TODO
        ...
        if (clash.second.first < b.Price) {
            ...
            if (/*b.Regions.size() <= 1.6*clash.first ||*/ random > 0.1) {
                ...
                return false;
                   ...
                   clash.second.first * 0.8) {  // multiplication factor
            // randomly chose one
            double random = ((double)rand() / (RAND_MAX));
        ...
        } else if (b.Price >= clash.second.first * 0.65 &&
                   regs_bid <= 0.9 * clash.first) {
            double random = ((double)rand() / (RAND_MAX));
...
long f(Bid b, Types::Price_Bids clash) {
    return (b.Price - 0.9 * clash.second.first) *
           pow((0.5 + (clash.first - b.Regions.size()) / M), 0.1);
}
