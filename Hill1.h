...
State HillClimb(State &s, int i, int cnt, int C, int passes,
    ...
    // long int max_clashamt_regs = 0; // find bid with minimum clash amt*(bid
    // regions - clash regions).
    Bid bestbid;
    ...
    if (random <= 0.0008) {
        ...
        if (max_profit == INT_MIN) {
            if (random <= 0.01) {
                Bid randomBid =
                    allBids[bidsofCompanyVec[randomBidId % noBids] % B];
                // std::cout << randomBid.Bid_Id << " is random ID \n";
                rand_clash = checkClashBidwithState(randomBid, s);
                addBidtoState(randomBid, s, rand_clash, true);
            }
...
void Restart_Hill(State &BestState) {
    ...
    while ((time(0) - Start_Time < 60 * Time - 2) && !Over) {
        ...
        Curr2 = HillClimb(Curr2, (i + 1) % C, 0, C, 5, rng);  // number of
                                                              // passes
        if (Curr2.Profit > BestState.Profit) BestState = Curr2;
        // std::cout << "Hill 1 :: " << Curr2.Profit << " Best yet of Hill1 : "
        // << BestState.Profit << std::endl;
        /*if (time(0)%60 ==0){
                        std::cout << "Hill 1 :: " << Curr2.Profit << " Best yet
        of Hill1 : " << BestState.Profit << std::endl;
        }*/
        // outfile <<
        // outfile << "\n \n \n";
        i++;
